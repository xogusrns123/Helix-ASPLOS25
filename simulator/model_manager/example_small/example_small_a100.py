...
class ExampleSmallA100(ModelOnMachine):
    def __init__(self) -> None:
        """
        This class stores the profiling results of running ExampleSmall on A100
        """
        # profiling results (for one single layer)
        self.prompt_bs2time: Dict[int, float] = {
            0: 0, 10: 50 * MilliSec, 20: 51 * MilliSec, 30: 51 * MilliSec, 40: 70 * MilliSec, 50: 90 * MilliSec
        }
        self.prompt_bs2vram: Dict[int, float] = {
            0: 0, 10: 10 * MB, 20: 20 * MB, 30: 30 * MB, 40: 40 * MB, 50: 50 * MB
        }
        self.decode_bs2time: Dict[int, float] = {
            0: 0, 1: 50 * MilliSec, 2: 51 * MilliSec, 3: 51 * MilliSec, 4: 70 * MilliSec, 5: 90 * MilliSec
        }
        self.decode_bs2vram: Dict[int, float] = {
            0: 0, 1: 10 * MB, 2: 20 * MB, 3: 30 * MB, 4: 40 * MB, 5: 50 * MB
        }

        # inference settings
        self.max_num_layers: int = 2
        self.num_layers_to_inference_settings: Dict[int, InferenceSettings] = {
            1: InferenceSettings(prompt_max_requests=4,
                                 prompt_max_tokens=30,
                                 prompt_typical_requests=2,
                                 prompt_typical_tokens=20,
                                 decode_max_context=30,
                                 decode_max_tokens=3,
                                 decode_typical_tokens=2),
            2: InferenceSettings(prompt_max_requests=3,
                                 prompt_max_tokens=30,
                                 prompt_typical_requests=2,
                                 prompt_typical_tokens=20,
                                 decode_max_context=30,
        ...
        self.kv_cache_capacity: Dict[int, int] = {1: 102400, 2: 51200}
        self.activation_backup_capacity: Dict[int, int] = {1: 102400, 2: 51200}
