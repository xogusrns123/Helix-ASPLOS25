...
from simulator.model_manager.example_large.example_large_t4 import ExampleLargeT4
from simulator.model_manager.example_large.example_large_a100 import ExampleLargeA100
from simulator.model_manager.example_large.example_large_h100 import ExampleLargeH100


class ExampleLargeStatistics(ModelStatistics):
    def __init__(self):
        """
        This class stores the profiling results of different machines running ExampleLarge
        """
        # machine profiling results
        self.h100 = ExampleLargeH100()
        self.a100 = ExampleLargeA100()
        self.t4 = ExampleLargeT4()

        # model statistics
        self.token_size: float = 8 * Byte
        self.activation_size: float = 8 * KB
        self.model_param_sizes = [1000.0 * MB, 1000.0 * MB, 1000.0 * MB, 1000.0 * MB, 1000.0 * MB, 1000.0 * MB,
                                  1000.0 * MB, 1000.0 * MB]
    ...
    def get_profiling_results(self, machine_type: str) -> MachineProfile:
        ...
        elif machine_type == "H100":
            return self.h100.get_profiling_results()
        else:
    ...
    def get_max_num_layers(self, machine_type: str) -> int:
        ...
        if machine_type == "A100":
            ...
            return self.h100.get_max_num_layers()
        else:
            assert False, "Found unknown machine type"
    ...
    def get_inference_settings(self, machine_type: str, num_on_node_layers: int) -> InferenceSettings:
        ...
        if machine_type == "A100":
            ...
            return self.h100.get_inference_settings(num_on_node_layers=num_on_node_layers)
        else:
            assert False, "Found unknown machine type"
    ...
    def get_typical_token_throughput(self, machine_type: str, num_on_node_layers: int) -> float:
        ...
        if machine_type == "A100":
            ...
            return self.h100.get_typical_token_throughput(num_on_node_layers=num_on_node_layers)
        else:
            assert False, "Found unknown machine type"
    ...
    def get_kv_cache_capacity(self, machine_type: str, num_on_node_layers: int) -> int:
        ...
        if machine_type == "A100":
            ...
            return self.h100.get_kv_cache_capacity(num_on_node_layers=num_on_node_layers)
        else:
            assert False, "Found unknown machine type"
    ...
    def get_activation_backup_capacity(self, machine_type: str, num_on_node_layers: int) -> int:
        ...
        if machine_type == "A100":
            ...
            return self.h100.get_activation_backup_capacity(num_on_node_layers=num_on_node_layers)
        else:
            assert False, "Found unknown machine type"
