...
from simulator.model_manager.llama2_70b.t4.llama2_70b_t4 import LLaMa70BonT4
from simulator.model_manager.llama2_70b.t4x2.llama2_70b_t4x2 import LLaMa70BonT4x2
from simulator.model_manager.llama2_70b.t4x4.llama2_70b_t4x4 import LLaMa70BonT4x4
from simulator.model_manager.llama2_70b.l4.llama2_70b_l4 import LLaMa70BonL4
from simulator.model_manager.llama2_70b.l4x2.llama2_70b_l4x2 import LLaMa70BonL4x2
from simulator.model_manager.llama2_70b.v100.llama2_70b_v100 import LLaMa70BonV100
from simulator.model_manager.llama2_70b.a100.llama2_70b_a100 import LLaMa70BonA100
from simulator.event_simulator.utils import kbps, mbps, gbps, Byte, KB, MB, GB, Sec, MilliSec, LLaMa2_70B_TOTAL_LAYERS


class LLaMa70BStatistics(ModelStatistics):
    def __init__(self, num_machines_dict: Dict[str, int]):
        """
        This class stores the profiling results of different machines running LLaMa70B
        ...
        normalized_perf_dict = {"A100": 80, "V100": 40, "L4": 20, "L4x2": 30, "T4": 12.5, "T4x2": 20, "T4x4": 30}
        ...
        for iteration in range(10):
            ...
            if "T4" in num_machines_dict:
                t4 = LLaMa70BonT4(num_machines_dict=num_machines_dict,
            ...
            if "T4x2" in num_machines_dict:
                t4x2 = LLaMa70BonT4x2(num_machines_dict=num_machines_dict,
                                      typical_layers_dict=typical_layers_dict,
                                      normalized_perf_dict=normalized_perf_dict)
            ...
            if "T4x4" in num_machines_dict:
                t4x4 = LLaMa70BonT4x4(num_machines_dict=num_machines_dict,
                                      typical_layers_dict=typical_layers_dict,
                                      normalized_perf_dict=normalized_perf_dict)
            ...
            if "L4" in num_machines_dict:
                l4 = LLaMa70BonL4(num_machines_dict=num_machines_dict,
                                  typical_layers_dict=typical_layers_dict,
                                  normalized_perf_dict=normalized_perf_dict)
            ...
            if "L4x2" in num_machines_dict:
                l4x2 = LLaMa70BonL4x2(num_machines_dict=num_machines_dict,
                                      typical_layers_dict=typical_layers_dict,
                                      normalized_perf_dict=normalized_perf_dict)
            ...
            if "V100" in num_machines_dict:
                v100 = LLaMa70BonV100(num_machines_dict=num_machines_dict,
                                      typical_layers_dict=typical_layers_dict,
                                      normalized_perf_dict=normalized_perf_dict)
            ...
            if "A100" in num_machines_dict:
                a100 = LLaMa70BonA100(num_machines_dict=num_machines_dict,
                                      typical_layers_dict=typical_layers_dict,
                                      normalized_perf_dict=normalized_perf_dict)
        ...
        if "T4" in num_machines_dict:
            self.t4 = LLaMa70BonT4(num_machines_dict=num_machines_dict,
                                   typical_layers_dict=typical_layers_dict,
                                   normalized_perf_dict=normalized_perf_dict)
        else:
            self.t4 = None
        if "T4x2" in num_machines_dict:
            self.t4x2 = LLaMa70BonT4x2(num_machines_dict=num_machines_dict,
                                       typical_layers_dict=typical_layers_dict,
                                       normalized_perf_dict=normalized_perf_dict)
        else:
            self.t4x2 = None
        if "T4x4" in num_machines_dict:
            self.t4x4 = LLaMa70BonT4x4(num_machines_dict=num_machines_dict,
                                       typical_layers_dict=typical_layers_dict,
                                       normalized_perf_dict=normalized_perf_dict)
        else:
            self.t4x4 = None
        if "L4" in num_machines_dict:
            self.l4 = LLaMa70BonL4(num_machines_dict=num_machines_dict,
                                   typical_layers_dict=typical_layers_dict,
                                   normalized_perf_dict=normalized_perf_dict)
        else:
            self.l4 = None
        if "L4x2" in num_machines_dict:
            self.l4x2 = LLaMa70BonL4x2(num_machines_dict=num_machines_dict,
                                       typical_layers_dict=typical_layers_dict,
                                       normalized_perf_dict=normalized_perf_dict)
        else:
            self.l4x2 = None
        if "V100" in num_machines_dict:
            self.v100 = LLaMa70BonV100(num_machines_dict=num_machines_dict,
                                       typical_layers_dict=typical_layers_dict,
                                       normalized_perf_dict=normalized_perf_dict)
        else:
            self.v100 = None
        if "A100" in num_machines_dict:
            self.a100 = LLaMa70BonA100(num_machines_dict=num_machines_dict,
                                       typical_layers_dict=typical_layers_dict,
                                       normalized_perf_dict=normalized_perf_dict)
        else:
            self.a100 = None
        ...
        self.activation_size: float = 8192 * 2 * Byte
        self.model_param_sizes = [1.53 * GB] * 80
        assert len(self.model_param_sizes) == LLaMa2_70B_TOTAL_LAYERS, "Total layer number mismatch!"
