...
def get_schedule(request_type):
    if request_type == "prompt":
        ...
        assert request_type == "prompt" or request_type == "decode", "wrong request type!!"

    return compute_node_uids, start_layers, end_layers

def run_disaggregate_host_online(
        # scheduler
        scheduler_name: str,
        # cluster
...
def run_disaggregate_host_offline(
        # cluster
        real_sys_config_file_name: str,
        # throughput
    ...
    """
    Run host with !!![Disaggregate + Offline mode]!!!.
    """
    print(f"Initializing host with disaggregate scheduling!")
    ...
    print(f'host_ip:{host_ip}')
    # assert host_ip.startswith("10"), "Local IP must be of form 10.xxx.xxx.xxx"
    llm_host.start_network_threads(CONFIG_BROADCAST_ADDR, host_ip, real_sys_config_file_name, "disaggregate")
    time.sleep(20)
    print("[Python] Cluster initialization finished!")
    # -------------------------------------------------------------------------------------- #
    ground_zero = time.time()
    ...
    while True:
        ...
        while not len(initial_requests) == 0 and initial_requests[0][0] <= now:
            ...
            compute_node_uids, start_layers, end_layers = get_schedule("prompt")
            # get query id
            cur_query_id = next_query_id
            next_query_id += 1
        ...
        for query_uid, route_list, num_layer_list in zip(finished_query_ids, routes, num_layers):
            ...
            if py_on_the_fly_query.processed_tokens == max_size:
                ...
                compute_node_uids, start_layers, end_layers = get_schedule("decode")
